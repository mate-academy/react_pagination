...
1. Show all the existing Items considering `total` and `perPage`
...
1. `«` and `»` links should open the prev and the next Items accordingly
...
1. Implement the `<select data-cy="perItemselector">` with `3`, `5`, `10`, `20` options to change the `perPage`;
...
- Replace `<your_account>` with your Github username in the [DEMO LINK](https://serhii-yunnikov.github.io/react_pagination/) and add it to the PR description.
