...
2. Show all the existing pages considering `total` and `perPage`
3. Current page should be highlighted with `li.active`;
4. `onPageChange` callback should be triggered only if page was changed;
5. The `App` should listen to the `onPageChange` and save a new page;
6. `«` and `»` links should open the prev and the next pages accordingly
    - disable each of them if it is already the first or the last page (use `li.disabled` and `a[aria-disabled="true"]`)
7. Show the pagination info inside `data-cy="info"` in the next format `Page 1 (items 1 - 5 of 42)`;
8. Implement the `<select data-cy="perPageSelector">` with `3`, `5`, `10`, `20` options to change the `perPage`;
    - show the 1st page after changing a `perPage`;
9. (*) Use React Router to save `?page=2&perPage=7` in the URL and apply them on page load
